...
[第 1 章](01-llm-call.md) で「LLM API はステートレスで、クライアント側が毎回 `messages` を丸ごと送る」と書いた。この章ではその `messages` の中身と、クライアントが持っている「状態 (state)」の正体を詳しく見る。
...
| `assistant` | LLM 自身 (過去の出力) | 応答テキスト、または `tool_calls` を伴うツール呼び出し | `content` が null で `tool_calls` だけのパターンもある ([第 4 章](04-tool-calling.md) で扱う) |
| `tool` | ツール実行の結果 | ツールが返した文字列 (JSON など) | `tool_call_id` で「どの tool_call への応答か」を紐付ける ([第 4 章](04-tool-calling.md) で扱う) |
...
ツール呼び出しが混じると `assistant` の `tool_calls` と `tool` role が配列に挟まるが、それは [第 4 章](04-tool-calling.md) で見る。
...
**API 呼び出し回数とターン数は別物**で、1 ターン内に API は 1 〜 N 回呼ばれる。この関係は第 5 章 (エージェントループ) で詳しく扱う。
...
## state と [第 2 章 コンテキストウィンドウ](02-tokens-context.md) の関係
...
## state と [第 6 章 記憶の多層モデル](06-memory.md) の関係

- この章の state = 第 6 章の **L3 (コンテキスト注入)** そのもの
...
- state は育ち続けるので、[context window 対策](02-tokens-context.md) と切り離せない
- 「同じ会話の継続」と「会話をまたいだ長期記憶」は別の層として整理する ([第 6 章](06-memory.md))
