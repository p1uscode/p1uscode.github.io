...
[第 13 章](13-system-prompt.md) の末尾で「system prompt だけでは安全性を守れない」と書いた。この章では「なぜ守れないか」 (=プロンプトインジェクション) と、「どう守るか」 (=ガード / Guardrails) を扱う。
...
[第 13 章](13-system-prompt.md) で「この制約を守れ」と書いても守れない理由:
...
エージェントの入出力フローに 4 箇所のガード層を挟む構造が基本 ([第 1 章](01-overview.md) の図):
...
- **監査 + 事後検知**: [第 8 章 Observability](08-observability.md) で全トレースを残し、異常を後から検知 + ロールバック可能に
