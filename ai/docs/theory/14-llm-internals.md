...
1. **Tokenizer** — テキスト → トークン ID 列 ([第 2 章](02-tokens-context.md))
...
ここまでが「学習データに含まれる事実 ≒ LLM の知識」の正体 ([第 0 章](00-overview.md) の Q6 "富士山の高さ" はここで覚えた)。
...
- **RAG** ([第 9 章](09-rag.md)) で正しい情報を context に注入
- **Citation 強制**: 出典を明記させる → 嘘は検証可能になる
- **「分からない」を許可**: system prompt で「不確かなら推測せず分からないと答えよ」と指示
- **評価** ([第 10 章](10-evaluation.md)) で hallucination 率を定期測定
...
章 00-13 で書いた内容と、この章の「中身」の対応:

| 章 | 中身との対応 |
|---|---|
| 01 ステートレス | 1 回の呼び出し = 1 回の「次トークン予測の連鎖」。サーバには何も残らない |
| 02 トークン | モデルの入力単位。Tokenizer で ID 化される |
| 04 Tool calling | instruction tuning で学習した「tools 形式の応答」ふるまい |
| 05 エージェントループ | 1 つの LLM 呼び出しは 1 回の autoregressive 生成。エージェントループは**外側で**複数の呼び出しを繋げているだけ |
| 06 記憶 L1 | pretraining で焼き込まれた事実 |
| 11 サンプリング | 次トークンの確率分布からどう選ぶかの制御 |
| 13 プロンプトインジェクション | LLM が指示とデータを区別できないのは、全部を同じ「文脈トークン列」として読むから |
...
- 章 00-13 の内容は全てこの「次トークン予測の連鎖」という骨格から発現している
