...
**API 呼び出し回数とターン数は別物**で、1 ターン内に API は 1 〜 N 回呼ばれる。この関係は第 7 章 (エージェントループ) で詳しく扱う。
...
## state と [第 8 章 記憶の多層モデル](08-memory.md) の関係

- この章の state = 第 8 章の **L3 (コンテキスト注入)** そのもの
- L5 (外部ストレージ / 長期記憶) は、**state に明示的に読み戻さない限り LLM に届かない**
...
- 「同じ会話の継続」と「会話をまたいだ長期記憶」は別の層として整理する ([第 8 章](08-memory.md))
