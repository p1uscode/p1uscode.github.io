...
プロンプトキャッシュ ([第 8 章](08-memory.md)) を使うとキャッシュヒット分は 50〜90% 割引される (Anthropic は最大 90% OFF、OpenAI は約 50% OFF)。長い system prompt が効くのはこれが理由。
...
- 高精度のマルチモーダル ([第 17 章](17-multimodal.md))
- 最新の tool-calling 精度
...
- 観測 / ログ ([第 9 章](09-observability.md)) を SaaS に送っていればそれも経路
...
対策は基本的に入力サニタイズ ([第 15 章](15-guards.md) の入力層ガード):
...
具体的なガード手法 (入力 / tool_calls / 結果 / 出力 の 4 層) は [第 15 章 ガードとプロンプトインジェクション](15-guards.md) に整理している。第 15 章は**インジェクション経由のツール悪用**が中心なのに対し、本節は**ツールそのものの出自と権限**に焦点。両方を組み合わせて初めて現実的なガード設計になる。
...
- **「クラウド = どれでも同じ」** → ベンダー / モデルごとに tool calling 精度、命令追従、コンテキスト効率が大きく違う ([第 12 章 評価](12-evaluation.md) で測るのが正しい)
