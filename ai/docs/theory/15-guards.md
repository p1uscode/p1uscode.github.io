...
[第 14 章](14-system-prompt.md) の末尾で「system prompt だけでは安全性を守れない」と書いた。この章では「なぜ守れないか」 (=プロンプトインジェクション) と、「どう守るか」 (=ガード / Guardrails) を扱う。
...
[第 14 章](14-system-prompt.md) で「この制約を守れ」と書いても守れない理由:
...
- **監査 + 事後検知**: [第 9 章 Observability](09-observability.md) で全トレースを残し、異常を後から検知 + ロールバック可能に
