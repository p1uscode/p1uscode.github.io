...
**RAG (Retrieval-Augmented Generation)** は「LLM の学習データに入っていない情報 (社内文書 / 最新情報 / 個別ユーザ情報など) を、クエリのたびに外部から取り出してプロンプトに注入する」パターン。[第 7 章 記憶の多層モデル](07-memory.md) の L4 (長期記憶) の**取り出し方**、[第 9 章 埋め込みと近傍検索](09-embeddings.md) の**使い所**がここで合流する。
...
ベクトル検索の上位 11〜51 件を、もう 1 段別のモデルで**並べ替える**。ベクトル検索は「類似性」を測るが、「この質問への関連性」はまた別物なので、精度を上げるには re-rank が効く。
...
- **プロンプトインジェクション経路**: 信頼できないソースをそのまま context に入れると攻撃経路になる ([第 14 章](14-guards.md))
