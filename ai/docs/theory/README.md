...
| **theory/** (ここ) | **座学** | 仕組み / 原理 / 設計の独立資料 (本ファイル群、01-20 章) |

**どこから始めてもよい**:

- **座学 → ハンズオン 派**: まず章 01-20 で仕組みを理解してから、[hands-on/](../hands-on/README.md) に進む。「全体像を掴んでから手を動かしたい」人向け
- **ハンズオン → 座学 派**: [setup/](../setup/README.md) でサービスを立ち上げ、[hands-on/](../hands-on/README.md) で触りながら、気になったところで対応する章を開く。「動いているものを触りながら理解したい」人向け
- **並行派**: 章 01-03 (見取り図と LLM の基本) を座学で読み、その後はハンズオンで動かしつつ、気になった章を拾い読みする
...
| # | トピック | 内容 | 状態 |
|---|---|---|---|
| 01 | [登場人物と責任範囲](01-overview.md) | LLM / エージェント / ツール / ガード / 人 の関係図と、LLM 単体の限界 (日時 / 天気 / 計算 等) を示すクイズ | ✅ |
...
| 18 | [ローカル LLM とクラウド LLM](18-local-vs-cloud-llm.md) | 2 つの選択肢、モデル形式 / ランタイム / 量子化 / ハード要件、クラウド課金 / レート制限 / retention、評価軸、使い分けの実務パターン、homelab のハイブリッド設計 | ✅ |
| 19 | [主要 AI ツールの全体像](19-ai-tools-overview.md) | LLM ベンダー純正 (Claude / ChatGPT / Gemini / Grok) × 配信形態 (Web / App / CLI) / サードパーティ (エディタ / ターミナル / 検索 / UI / ワークフロー / 観測 / フレームワーク / ローカル)、評価軸、homelab の立ち位置 | ✅ |
