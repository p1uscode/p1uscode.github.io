...
- **ただし絶対的な権威ではない**。user や tool 結果に悪意のある文字列が入れば system 指示を覆される可能性がある (= プロンプトインジェクション、[第 15 章](15-guards.md))
...
ただしこれは**プロンプトインジェクションに完全には耐えられない** ([第 15 章](15-guards.md))。別途ガード層が必要。
...
動的情報を system prompt に入れることで、ユーザ体験をパーソナライズできる (これが**コンテキストエンジニアリング** — [第 18 章](18-engineering-layers.md))。
...
2. **失敗例を集める** ([第 9 章 Observability](09-observability.md) のトレースを眺める)
3. **失敗パターンごとに指示を追加**
4. **回帰を確認** ([第 12 章 評価](12-evaluation.md) のデータセットで再評価)
...
- system prompt ≠ エージェント全体の設計。context engineering と harness engineering と組み合わせて初めて品質が出る ([第 18 章](18-engineering-layers.md))
