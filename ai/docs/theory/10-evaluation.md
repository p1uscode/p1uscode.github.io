...
1. **本番トレースから抽出** ([第 7 章 Observability](07-observability.md) の Langfuse トレースが素材になる)
...
ここまでやって初めて「エージェントを改善する」サイクルが回る。**観測 ([第 7 章](07-observability.md)) と評価 (この章) は車の両輪**で、片方だけでは運用できない。
...
- **観測 ([第 7 章](07-observability.md)) と評価はセット**。観測だけではデバッグしかできず、評価だけでは対象が無い
