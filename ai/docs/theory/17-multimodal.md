...
ここまでは「LLM = テキスト in / テキスト out」で話を進めてきた。現実には AI モデルは**画像・音声・動画・ベクトル**等様々なモダリティを扱っている。この章では主要な「他のモデル」を整理し、本リポジトリの 01-16 章の知識が**そのまま応用できる**ことを示す。
...
| カテゴリ | 入力 | 出力 | 例 |
|---|---|---|---|
| **LLM** | テキスト | テキスト | GPT-5.5 / Claude / Gemini (テキストモード) |
| **VLM** (Vision-Language Model) | 画像 + テキスト | テキスト | GPT-5.5 / Claude Opus 4.8 / Gemini 3.1 |
...
| **Omni-modal** | 任意 | 任意 | GPT-5.5 / Gemini 3.1 / Claude Opus 4.8 |
...
omni-modal LLM (GPT-5.5 / Gemini Live) はテキストを介さず直接音声入出力をサポート。WebSocket 経由で streaming:
...
[第 10 章 埋め込み](10-embeddings.md) で扱った。text-embedding-3 / BGE / CLIP 等。
...
- 章 02 の API 形式はそのまま
- 章 04 の messages / state もそのまま (content が配列化されるだけ)
- 章 05 の tool calling もそのまま
- 章 07 のエージェントループもそのまま
- 章 09 の observability もそのまま (span の中身に image_url 等が入るだけ)
...
- **omni-modal モデル** (GPT-5.5 / Gemini / Claude Opus 4.8) は単一モデルで複数モダリティを扱う。リアルタイム音声対話も可能
- **「エージェント = コアモデル + ツール + state」という骨格はどのモダリティでも同じ**
- 章 01-16 で学んだ知識は全てそのまま他モダリティに応用できる
