...
現実的には judge には GPT-5.5 / Claude Opus クラスの強いモデルを使い、judge 自体の評価は人手サンプリングで定期的に校正する。
...
1. **本番トレースから抽出** ([第 9 章 Observability](09-observability.md) の Langfuse トレースが素材になる)
...
1. Langfuse UI で 20〜50 件ほど「良いターン」「悪いターン」にスコア付け
2. それを Dataset として切り出し
3. system prompt を変えて Dataset を再実行
4. 人手 + LLM judge でスコア比較

ここまでやって初めて「エージェントを改善する」サイクルが回る。**観測 ([第 9 章](09-observability.md)) と評価 (この章) は車の両輪**で、片方だけでは運用できない。
...
- **観測 ([第 9 章](09-observability.md)) と評価はセット**。観測だけではデバッグしかできず、評価だけでは対象が無い
