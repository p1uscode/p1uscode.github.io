...
| # | 章 | 見るもの |
|---|---|---|
| 02 | LLM の 1 回の呼び出し | `POST /v1/chat/completions` 1 回のリクエスト / レスポンスの中身を素で見る |
| 03 | トークンとコンテキストウィンドウ | LLM が文字列をどう区切って数え、何文字まで覚えていられるか |
| 04 | Messages と state | 会話履歴の配列構造、role の意味、「1 ターン」の定義 |
| 05 | Tool calling | 上の図の「LLM ↔ エージェント ↔ ツール」の往復が具体的にどんな JSON で表現されるか |
| 06 | エージェントループ | 04 と 05 を組み合わせて「ツール呼出 → 実行 → 結果 → 再推論」を回す制御フロー |
| 07 | [記憶の多層モデル](07-memory.md) | 「LLM が前の話を覚えている」ように見えるものの実体 |
| 08 | Observability / tracing | エージェント内部の各ステップを後から追えるようにする仕組み (Langfuse / OTel) |
| 09-11 | 埋め込み / RAG / 評価 | 外の知識を注入する仕組みと、その品質を測る方法 |
| 13 | プロンプト設計 | LLM に「いつ何をすべきか」を正しく伝える方法 |
