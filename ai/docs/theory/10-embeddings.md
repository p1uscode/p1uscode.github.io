...
[第 8 章 記憶の多層モデル](08-memory.md) の L5 (外部ストレージによる長期記憶) と、次章で扱う **RAG (Retrieval-Augmented Generation)** の両方に共通する基盤技術が「埋め込み (embedding)」と「近傍検索 (nearest neighbor search)」。この章ではその仕組みを最短距離で説明する。
