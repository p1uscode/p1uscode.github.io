...
どれも**内部的には OpenAI 互換 API で複数モデルを切り替える**ことが多い。中身は LLM + tool calling + RAG (コードベース) + エージェントループの組み合わせで、章 01-09 の知識がそのまま通用する。
...
共通点: 非エンジニアでもエージェント / RAG / ワークフローが組める GUI。内部では章 04-09 の仕組みを抽象化しているだけ。

### (6) オブザーバビリティ / 評価

| 製品 | 特徴 |
|---|---|
| **Langfuse** (**本リポジトリ**) | OSS、OTel ネイティブ、セルフホスト可能、[第 7 章](07-observability.md) で使用 |
...
共通点: 章 04 tool calling / 05 エージェントループ / 03 state management を抽象化。どのフレームワークも根っこは同じ (第 16 章 ハーネス層)。
...
`homelab/` + `examples/agent-demo/` は、上のプロダクト群が**裏で使っている仕組みの最小実装**として位置付けられる。たとえば Cursor も Claude Code も Dify も n8n も、中身は:

- LLM 呼び出し ([第 01 章](01-llm-call.md))
- トークン / コンテキスト管理 ([第 02 章](02-tokens-context.md))
- messages / state 管理 ([第 03 章](03-messages-state.md))
- tool calling ([第 04 章](04-tool-calling.md))
- エージェントループ ([第 05 章](05-agent-loop.md))
- 記憶 / RAG ([第 06, 08, 09 章](06-memory.md))
- 観測 ([第 07 章](07-observability.md))
- 評価 ([第 10 章](10-evaluation.md))
- プロンプト / コンテキスト / ハーネス の 3 層 ([第 16 章](16-engineering-layers.md))

を組み合わせて製品化しただけ。**homelab で学んだ知識はそのまま全プロダクトの理解に通じる**。逆に言うと、どれか 1 つに詳しくなっても本質は学べず、原理 (章 00-17) を押さえた上で全プロダクトを横断的に評価できるのが理想。
...
だから**個別プロダクトを追うより、仕組み (章 00-17) を押さえた方が長く使える**。新しいプロダクトが出てきたときも、「これは LLM + RAG + tool calling を組み合わせたやつか」と即座に分類できる。
...
- **本リポジトリ** (homelab + agent-demo) は全プロダクト共通の基盤の最小実装。ここで学んだ知識はそのまま他プロダクトに通用する
- プロダクト評価軸: モデル選択 / プロンプト自由度 / コンテキスト能力 / ハーネス成熟度 / セキュリティ / 価格
- **個別プロダクトより仕組みが長持ち**。章 00-17 の原理を押さえた上で全体像を眺めるのが本質的な理解
