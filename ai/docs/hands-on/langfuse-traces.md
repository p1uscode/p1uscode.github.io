...
ログイン後、左メニューから **`p1uscode` 組織 → `default` プロジェクト**を選ぶ。
...
これが [theory 03 トークンとコンテキストウィンドウ](../theory/03-tokens-context.md) で書いた「モデル間でトークン数がずれる」の実機確認。
...
2 件目の Input を開いて `messages` を確認すると、messages 配列に過去のやり取りが全部詰まっていることが確認できる。これが「state = messages 配列」の目で見える形 ([theory 04](../theory/04-messages-state.md))。
...
Traces タブで上部のフィルタ条件に `tags` contains `agent-demo` を入れると、agent-demo からのトレースだけに絞り込める (agent-demo 側で `CallbackHandler({ tags: ["agent-demo", mode] })` と付けているため。`mode` は `single-shot` / `interactive`)。agent-demo はまだ動かしていないので結果 0 件のはず — ハンズオン 4 でもう一度試す。
...
今はまだ agent-demo が動いていないので [ハンズオン 4](agent-demo.md) で改めて確認する。
